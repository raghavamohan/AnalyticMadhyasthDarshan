...
class ContributorTests(unittest.TestCase):
    ...
    def test_dashboard_refresh_checks_public_publication(self):
        ...
        self.assertIsNotNone(soup.find(id='dashboard-refresh-text'))
        self.assertIsNotNone(soup.find(id='amd-portal-wait'))
        self.assertIsNotNone(soup.find(id='theme-toggle-label'))
        self.assertIsNotNone(soup.find(id='auth-kind-note'))
        self.assertEqual(soup.find(id='hub-back-link').name, 'div')
        self.assertNotIn("getElementById('hub-back-link').hidden", portal)
