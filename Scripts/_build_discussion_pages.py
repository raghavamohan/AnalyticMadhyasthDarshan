...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .compose-trigger:hover {{ background: var(--accent-soft); }}
...
<div class="wrap">
  ...
  <section id="compose-panel" aria-label="Add a comment">
    <div id="compose-wrapper" class="compose-wrapper">
      <div id="compose-trigger" class="compose-trigger" role="button" tabindex="0" aria-expanded="false" aria-controls="compose-expanded">
        What are your thoughts on this study?
      </div>
      <div id="compose-expanded" class="compose-expanded-content">
        <div id="auth-signed-in" class="auth-status-inline hidden">
          <span>Signed in as <strong id="auth-user-label"></strong></span>
        </div>
        <div id="auth-signed-out">
          <p class="auth-sign-in-note">Sign in to post. We will send a one-time link to your email.</p>
...
(() => {{
  ...
  const expandCompose = ({{ focusLogin = true }} = {{}}) => {{
    if (!composeExpanded) {{
      ...
      composeWrapper.classList.add("is-expanded");
      composeTrigger.setAttribute("aria-expanded", "true");
    }}
  ...
  composeTrigger.addEventListener("click", () => expandCompose());
  composeTrigger.addEventListener("keydown", (event) => {{
