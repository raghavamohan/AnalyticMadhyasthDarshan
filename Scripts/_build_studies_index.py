...
(function(){
  ...
  try {
    window.__amdGithubAuthPrefetch=fetch("https://api.analyticmadhyasthdarshan.org/api/auth/me",{
      ...
      if(data.loggedIn&&typeof data.login==="string"&&data.login){
        snapshot.login=data.login;snapshot.userId=data.userId;
        if(data.notifications&&typeof data.notifications.configured==="boolean"){
          snapshot.notifications={
            configured:data.notifications.configured,
            hasEmail:Boolean(data.notifications.hasEmail),
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .search-passage-hint.is-emphasized { color: var(--text); font-weight: 600; }
  .search-passage-hint a { font-weight: 600; }
  .seg-label {
...
<section class="section" id="studies">
  ...
  <p class="search-passage-hint" id="search-passage-hint">Looking for a specific phrase in the full text? <a href="search.html">Passage Search</a></p>
...
(() => {
  ...
  // Catalog timestamps have an explicit IST contract; do not delegate this
  // human-readable format to the browser's locale-dependent Date.parse.
  const catalogTimestamp = updated => {
    const m = String(updated || "").match(/^([A-Za-z]{3}) (\\d{1,2}), (\\d{4}), (\\d{1,2}):(\\d{2}) (AM|PM) IST$/);
    if (!m) return NaN;
    ...
    if (month < 0 || day < 1 || hour < 1 || hour > 12 || minute > 59) return NaN;
    const local = new Date(Date.UTC(year, month, day, hour % 12 + (m[6] === "PM" ? 12 : 0), minute));
    if (local.getUTCMonth() !== month || local.getUTCDate() !== day) return NaN;
    return local.getTime() - 330 * 60000;
  };
  const ts = s => {
    const value = catalogTimestamp(s.updated);
  ...
  const pdfVersionQuery = updated => {
    if (!updated) return "";
    const t = catalogTimestamp(updated);
  ...
  const renderCatalog = () => {
    ...
    const passageHint = document.getElementById("search-passage-hint");
    if (passageHint) passageHint.classList.toggle("is-emphasized", Boolean(state.q) && shown === 0);
  ...
  if (startHere) {
    startHere.addEventListener("click", e => {
      ...
      const panel = startHere.querySelector(`.path-panel[data-stage="${go.dataset.goStage}"]`);
  ...
  const syncStartHere = studies => {
    ...
    document.querySelectorAll("#start-here [data-study-slug]").forEach(item => {
      ...
      const links = own("[data-study-link]");
      if (item.matches("[data-study-link]")) links.unshift(item);
      links.forEach(studyLink => {
