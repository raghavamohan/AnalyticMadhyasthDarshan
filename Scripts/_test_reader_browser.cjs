...
(async () => {
  ...
  try {
    ...
    const utteredSentence = await page.evaluate(() => window.testSpeech.spoken.at(-1).text.trim());
    const expectedSentences = [...new Intl.Segmenter('en',{granularity:'sentence'}).segment(paragraphText)].map(s => s.segment.trim());
    assert.ok(expectedSentences.includes(utteredSentence),'visible-position reading begins and ends at full sentence boundaries');
    assert.equal(await page.$eval('#reader-tools',n => n.open),false);
    ...
    await page.click('#reader-read');
    assert.equal(await page.evaluate(() => window.testSpeech.spoken.at(-1).text.trim()),utteredSentence,'Resume restarts the complete sentence');
    await page.evaluate(() => window.testSpeech.spoken.at(-1).onstart());
    await page.click('#reader-read');
