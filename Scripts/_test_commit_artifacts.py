...
def run_case(head_repo: str, *, dirty: bool) -> tuple[int, str, bool]:
    """Run the step once.
    ...
    treated as having regenerated something.

    Returns the exit status, the combined output, and whether the remote moved.
    """
    ...
    try:
        ...
        return done.returncode, done.stdout + done.stderr, before != after
    finally:
        shutil.rmtree(tmp, ignore_errors=True)


def test_same_repo_pull_request_still_pushes() -> None:
    code, out, pushed = run_case(THIS_REPO, dirty=True)
    assert code == 0, out
...
def test_fork_pull_request_with_stale_artifacts_fails_with_guidance() -> None:
    ...
    code, out, pushed = run_case(FORK_REPO, dirty=True)
    assert code == 1, out
...
def test_fork_pull_request_with_correct_artifacts_passes() -> None:
    ...
    """
    code, out, pushed = run_case(FORK_REPO, dirty=False)
    assert code == 0, out
    assert not pushed
...
def test_event_without_pull_request_payload_still_pushes() -> None:
    ...
    code, out, pushed = run_case("", dirty=True)
    assert code == 0, out
