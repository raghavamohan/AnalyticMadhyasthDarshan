...
- **Shared glossary tooltips are reader-only** — `Studies/glossary.json` decorates
  the published HTML reader, while each study's printable glossary remains in its
  canonical markdown. `_html_to_pdf.js` unwraps the web-only tooltip markup before
  printing, so shared-glossary edits must not trigger or invalidate PDF builds.
  After editing the shared glossary, run `python Scripts/_sync_glossary_html.py --write`
  and then `python Scripts/_build_reader_offline.py`; do not regenerate study PDFs.
...
math, and printable glossary content are not recovered from PDF — re-add them in markdown.
...
Removing selected companions uses
[remove-study-companions](.agents/skills/remove-study-companions/SKILL.md) and
`Scripts/_remove_study_companions.py`; the same scoped cleanup runs in CI
preparation. Deleting a deck also removes its declared presenter-companion chain.
Deleting a note or deck never invokes whole-study retirement. Shared figures,
canonical study files, catalog/proposal metadata and parent timestamps remain
unless separately edited. Repair inbound links in the same PR.
