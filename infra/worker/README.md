...
- **API catalog:** [RFC 9727](https://www.rfc-editor.org/rfc/rfc9727) discovery at `/.well-known/api-catalog` is served by Worker `amd-api-catalog` on a zone route ([`infra/api-catalog-worker`](../api-catalog-worker/README.md)). Agent Skills Discovery at `/.well-known/agent-skills/` is served by Worker `amd-agent-skills` on a zone route ([`infra/agent-skills-worker`](../agent-skills-worker/README.md)). The MCP Server Card at `/.well-known/mcp/server-card.json` and `/mcp` plus the public study APIs are served by Worker `amd-mcp` on zone routes ([`infra/mcp-worker`](../mcp-worker/README.md)). Web Bot Auth at `/.well-known/http-message-signatures-directory` is served by Worker `amd-web-bot-auth` on a zone route ([`infra/web-bot-auth-worker`](../web-bot-auth-worker/README.md)). Auth.md at `/auth.md` is served by Worker `amd-auth-md` ([`infra/auth-md-worker`](../auth-md-worker/README.md)). In-browser catalog tools are registered by [`/webmcp.js`](../../webmcp.js). The DNS-AID ServiceMode HTTPS record at `_index._agents` is published with [`Scripts/_publish_dns_aid.py`](../../Scripts/_publish_dns_aid.py) and signed with DNSSEC. The homepage advertises only those implemented surfaces, catalog JSON, OpenAPI, and docs through RFC 8288 `Link` headers. Identity policy: [`/auth.md`](../../auth.md). Human docs: [`api-docs.html`](../../api-docs.html). Human write APIs use first-party session cookies and do not advertise bearer authentication.
