...
**Edited on:** July 3, 2026, 10:27 AM IST
**Status:** Released
...
| Activity triad | **D1b** | Petri/monoidal transitions (§6.10): firing carries effort, motion, and result jointly; inputs include other units' outputs (mutuality); *kshobh* = enabled transition that fails to fire | Strong for bookkeeping; the joint-form loop is transition labelling, not arrows |
...
This layer is the categorical home of the template's **activity triad** (template D1b): a firing carries effort, motion, and result *jointly* — effort consumed as input, result produced as output, the firing itself the motion — and φ reads value off the produced token. The Petri semantics is native to the triad's mutuality coupling: a transition's inputs legitimately include *other* units' outputs, which is exactly the "pressure of mutuality" the template's D1b cites (MVD p. 114) — one atom's expelled result enabling another's bonding. What the model does not carry is the triad's joint-form constitution — each moment the outcome of the other two (MVD p. 105) — which is a symmetric relation among aspects of one activity, not a sequence; it enters as transition *labelling*, not as composable arrows. Frustration (*kshobh*, template D5a, D1b) is an enabled transition that fails to fire: inputs present, output never produced — the triad not closing toward restfulness. The stage mapping above is D1b's completeness reading of the same triad.
...
| Activity triad (D1b) | Petri transition firing carrying all three moments; mutuality as cross-unit inputs; φ reads the token; *kshobh* = enabled but unfired (§6.10) | Moderate | Exchange and coupling captured; the joint-form loop and goal-directedness are textual content carried as labelling |
...
Categorical honesty: this is a **family** of functors and partial composites across `Eval`, `Conduct`, `Ev`, and `Trans` — not `id_Liv`. Transmission τ at the knowledge order must carry **evidenced** understanding (P16), linking the tradition step to education-*sanskar* (§6.8). One further register belongs here: *chitta*'s eight activities of visualisation include effort, motion, and result (MVD p. 327), so the activity triad (template D1b) is not only ontic process (§6.10) but a D12 epistemic category — part of the vocabulary in which *jeevan* grasps any object it evaluates.
...
- **MVD** — Nagraj, A. [*Madhyasth Darshan — Co-existentialism*](../References/Madhyasth-Darshan/MVD-Madhyasth-Darshan-Coexistentialism.pdf). English translation by Rakesh Gupta. Cited: Realisation Knowledge (p. 11); evidence chain and "Believe what is known" (p. 12); four orders; unit signature (pp. 50–51); relationships and values; capacity–ability–receptivity (pp. 62, 79, 134, 142); mixture and compound (p. 42); hungry/overfull atoms (p. 8); frustration of effort as yearning for restfulness (p. 104); joint form of effort, motion, and result (p. 105); mutuality pressure in results (p. 114); knowledge unfolding (pp. 115–116, 289); realisation in coexistence (p. 116); three satisfactions (Ch. 4); organisation sustainment (p. 55); *chitta*'s eight visualisation activities (p. 327); samadhi-samyama warrant (p. 7).
...
- [*Coexistence From First Principles*](../Coexistence-From-First-Principles/Coexistence-From-First-Principles.pdf) — the generative kernel and clause-level recovery audit that executes the §6.16.4 programme; operadic sketch as successor to the §6.8–6.10 colimit vocabulary
