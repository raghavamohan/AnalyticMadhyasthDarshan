...
**Edited on:** August 19, 2026, 2:31 PM IST
**Status:** Draft
...
| Complementary mutuality | Need met by need across units — hungry with overfull, nourishment with eater, relationship with relationship — without converting one order into another. |
...
```mermaid
flowchart TD
    SATTA["Saturation in satta"] --> UNIT["Active unit"]
    UNIT --> DHARMA["Dharma: potential Phi / attractor"]
...
| Plant or animal body | Organism composed of cells | Pranic | Seed-conformance |
| Animal joint form | Body with *jeevan* | Animal | Species-conformance |
| Human joint form | Developed body with *jeevan* | Knowledge | *Sanskar*-conformance |

The table is this paper's reconstruction of the sequence and the four-order partition. It is not a further completeness threshold numbered after T1. A plant body is a pranic composition; it is not *jeevan*. An animal or human is a joint form in which the compositional line supplies the body and the atomic line supplies constitutionally complete *jeevan* (§6). Most atoms remain engaged in physical and chemical constitution rather than attaining T1. Biological compositions return to material constituents after disintegration (MVD, pp. 8, 13; JV, pp. 47–48).
...
flowchart TD
    ...
    CELL --> BODY["Plant or animal body"]
    DEV["Atomic development"] --> T1["T1: jeevan"]
    BODY --> ANIMAL["Animal joint form"]
    T1 --> ANIMAL
    ANIMAL --> HUMAN["Human joint form"]
    T1 --> HUMAN
```

### 4.4 Nested dependence

A plateau is not only a description of what has already closed. It is a **constitutive condition** for the next closed unit. Molecules form when atoms of the required kinds are already present and complementary; compounds and minerals require those molecules; pranic cells require chemical substances, definite heat, and the inherent composition-method of the *prana sutra*; plant and animal bodies require cells; the animal joint form requires a body at that plateau together with a T1 *jeevan*; the human joint form requires the further developed bodily medium through which knowing, believing, recognising, and fulfilling can be evidenced (KD 3.2, pp. 58–60; JV, pp. 47–48, 59).

In the reconstruction, formation of a unit in *P<sub>ℓ+1</sub>* is admitted only when *P<sub>ℓ</sub>* is already occupied under conducive conditions. That dependence is existential progression (*niyati-kram*): the definite order in which the four orders become manifest, not a claim that every instance advances (MVD, pp. 8, 13). The way of existence (*niyati-vidhi*) is the conformance by which each plateau maintains definite conduct once formed. The two must not be collapsed into T1. Constitutional completeness is development in the atom; the plateaus are development in composition. They meet in the animal and human joint forms. They are not one ladder that a single *x*(*t*) climbs by accumulating attributes until it becomes *jeevan*.
...
**Use, nourishment, and participation** is fulfilment across orders without identity of order. Pranic units take material compositions as nourishment-element and composition-element under seed-conformance. Animals select and taste through a species-limited body. Humans can know, believe, recognise, evaluate, and fulfil with material, pranic, animal, and human units (JV, pp. 47–48, 69). Knowledge-order use of lower orders is bounded by right-use and regeneration; extraction beyond regeneration is an unfulfilled coupling that destabilises the containing assembly (MVD, p. 264; JV, p. 77).

The arrows below are couplings in mutuality, not conversions of order.

```mermaid
flowchart LR
    MAT["Material units"] ---|complement, nourish| PRAN["Pranic units"]
    PRAN ---|body, habitat| ANIM["Animal joint forms"]
    PRAN ---|body, habitat| HUM["Human joint forms"]
    MAT ---|contain, right-use| HUM
    ANIM ---|species mutuality| ANIM
    HUM ---|relationship, work| HUM
...
In the reconstructed state space, the continuous physical coordinates of particle-exchange collapse into an invariant structural core, and a further set of **cognitive coordinates** x<sub>cognitive</sub> becomes the relevant description of activity. The unit is now *jeevan*: a constitutionally complete, immortal, sentient atom that operates through a physical body and is not reducible to the laws of molecular and weight bondage. The body through which it operates is still a compositional plateau (§4.3). Whether and how that threshold is to be identified in contemporary physics remains an open problem (§11).
...
**x(t) = [ x<sub>physical</sub>(t) , x<sub>cognitive</sub>(t) ]<sup>T</sup>**
...
In the unawakened human state the reconstructed cognitive potential Φ<sub>cognitive</sub> is misaligned. The upper faculties (*buddhi* and *atma*) remain unevidenced. The derivative ẋ<sub>cognitive</sub> is driven by feedback from the body's sensory inputs, seeking happiness through sensory pleasure, profit, and temporary comfort (SB, pp. 91–92; MVD, p. 78).
...
**Internal friction (ẋ<sub>cognitive</sub> ≠ 0) ⇒ psychological division / suffering**
...
Activity completeness is reached when *atma* directly perceives coexistence (*anubhav*). That realisation is the restfulness of effort (*shram ka vishram*): the restless search for certainty stabilises into knowing (SB, p. 58; MVD, Ch. 5). In the reconstruction, T2 is the fixed point at which ẋ<sub>cognitive</sub> = 0 in the sense of inner coherence, not in the sense that *jeevan* becomes inactive. Motion continues; confusion does not.
...
The nested regions *P<sub>ℓ</sub>* and the table in §4.3 reconstruct compositional sequence and order-partition. They are not a fourth completeness stage and are not numbered T0. Composition closes a new bounded unit; development in the atom reaches T1. The two lines meet in animal and human joint forms. The paper does not assert that every instance advances, or that a plant is a deficient animal.
...
- **JV** — Nagraj, A. [*Jeevan Vidya: An Introduction*](../References/Madhyasth-Darshan/JV-Jeevan-Vidya-An-Introduction.pdf). English translation by Rakesh Gupta. Cited: nothing isolated, complementarity in mutuality (pp. 43, 69; §2.3, §5); four orders and conformance (pp. 47–48; §4.3–§4.4, §5.2); inclination toward coexistence (p. 67; §5.2); relationship with the Earth (p. 77; §5.2); human bodily medium (p. 59; §7); ten activities (pp. 92–94; §7); delusion and four-and-a-half effective activities (pp. 93–94; §7); human goals (p. 61; §10).
