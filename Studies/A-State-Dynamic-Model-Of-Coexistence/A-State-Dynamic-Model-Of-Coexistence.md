...
**Edited on:** September 1, 2026, 10:44 AM IST
**Status:** Released
...
```mermaid
flowchart TB
    ATMA["Atma<br/>realisation (state) · evidence (motion)"]
    BUDDHI["Buddhi<br/>enlightenment (state) · resolve (motion)"]
    CHITTA["Chitta<br/>contemplation (state) · imaging (motion)"]
    VRITTI["Vritti<br/>comparison (state) · analysis (motion)"]
    MUN["Mun<br/>tasting (state) · selection (motion)"]
    BODY["Pranic body<br/>sensitivity · bodily expression"]
    ATMA -->|"higher-conformance"| BUDDHI
    BUDDHI --> CHITTA
    CHITTA --> VRITTI
    VRITTI --> MUN
    MUN -->|"selection expressed through the body"| BODY
    BODY -.->|"lower-conformance: pressure from vitality, senses, body"| MUN
```
