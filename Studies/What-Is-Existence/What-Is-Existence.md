...
Picture two inseparable poles always together: a formless, actionless ground and countless bounded units whose activity constitutes all change. The ground does not transform; units do. Nothing in nature is isolated; every unit is saturated in Omnipresence and stands in definite relationships with others. Development is not creation from nothing but transformation within this beginningless whole — from insentient atom through constitutional completeness to sentient *jeevan*, and in humans through awakening toward conduct that evidences coexistence. The sections below follow that arc from the ground inward.
...
Every unit saturated in Omnipresence carries the same four inseparable aspects — form, properties, essential nature, and *dharma* — regardless of order (see the ontology diagram, §1). MVD states that all four orders have differentiating features related to these four (MVD, pp. 50–51). They are not optional descriptors added by a knower; they are what each unit **is** as a participant in coexistence.
...
SB presents mutual dependence as a continuous identity-chain — a characteristic Madhyasth Darshan reasoning pattern in which each link is an identity ("itself is"), not an external push:
...
*Satta*'s uniform energy becomes forcefulness only through the unit, becomes activity only through basic impulsion, becomes development only through effort–motion–result — the triad developed in §1.6.
...
Nature exists as countless physical and sentient units — organised, bounded, and active within Omnipresence. The ontology diagram (§1) maps these units to four **ontological orders** — material, pranic/bio, animal, and knowledge/human — each a stable plateau in development. The table in §1.2 gives each order's essential nature and cumulative *dharma*; this section states how the orders relate.
...
Development within saturation is oriented toward completeness. The ontology diagram (§1) shows three completeness stages — constitutional, activity, and conduct — mapped to plane transitions T1–T3. This section traces **Development Progression** through T1; awakening milestones within constitutionally complete *jeevan* are §1.9.
...
Two conservation claims must be kept apart.

**Coexistence conservation.** Madhyasth Darshan holds that what exists does not arise from non-being (*abhava*) and does not vanish into it. This ontology distinguishes absolute annihilation (*vinasha*) from physical transformation (*roopantaran*). An entity changes configuration, but its underlying substance (*vastu*) is never destroyed:
...
From this follow: (1) existence (*astitva*) as coexistence (*saha-astitva*) is beginningless and endless; (2) Omnipresence (*satta*) does not begin, change, or end; (3) units (*ikai*) do not arise from *abhava* and do not vanish into it; (4) particular bodies (*shareer*), configurations, relations, and states do begin and end; (5) development (*vikas*) and awakening (*jagriti*) are real processes within beginningless coexistence. This picture is **consonant with** careful readings of conservation in physics when configuration changes while quantities persist (§4.3) — consonant with, not supported by.

***Jeevan* persistence, rebirth, and transmigration.** The texts infer that *jeevan* — an immortal sentient unit whose constitutional completeness does not lapse at bodily death — continues after bodily death as it does while driving a body, and may **re-associate** with a new body (JV, p. 20; §1.9; MVD Reality proposition 9). That is a **rebirth / transmigration** commitment within the darshan. It **does not follow** from physics-style conservation alone. Science conserves mass-energy and charge, not particular composite configurations or functional identities. JV's charcoal analogy shows that *vastu* survives transformation; it does not by itself prove that **this** constitutionally complete sentient configuration persists as the same *jeevan* across a gap of embodiment. Post-death continuity and re-association therefore rest on separate commitments: *gathanpurnata*, functional indivisibility of the sentient atom, and text-based philosophical inference — not on thermodynamic or field-theoretic conservation theorems (§6.2.4). They are subject to the same evidential bar as other claims exceeding public science (§6.4).
...
The table compares primary entities across the four traditions this paper develops in the body. Abrahamic and Dvaita ontologies are outside scope for the table. **Buddhist** traditions are likewise excluded from the entity matrix — this paper does not develop a full Buddhist comparative section — but **anatta** (no enduring self) and Madhyamaka *svabhava* critique are central challenges to immortal distinct *jeevan* and are engaged in §6.2.2. Nagarjuna appears there because mutual dependence in saturation invites the same question Madhyamaka presses against intrinsic existence; excluding Buddhism from §5.4 is a scope choice about table width, not a judgment that Buddhist philosophy is irrelevant to the ontology.
...
If *jeevan* is a constitutionally complete atom that **drives** the body, physicalist readers face an interaction problem as old as Descartes. Either *jeevan* exerts physical force on neural and muscular tissue — in which case the interaction should be measurable in principle, and constitutional completeness should be operationalizable, contradicting §6.2.7 — or it does not exert physical force, in which case "drives" must name a non-mechanical mode of influence that the texts have not specified. Madhyasth Darshan treats the body as medium and instrument (§1.9.1); MVD describes a vibrational pathway from cognitive organs to knowledge-unfolding (§1.9.5). None of that yet answers causal closure of the physical: how intention in *jeevan* becomes bodily motion without either violating physics or reducing *jeevan* to epiphenomenal accompaniment. This gap is not a minor detail for readers trained in neuroscience; it deserves explicit treatment in the planned *Philosophy-Of-Mind-And-Jeevan* study.
