...
**Edited on:** July 3, 2026, 12:13 PM IST
**Status:** Draft
**The thesis:** Madhyasth Darshan's ontology can be generated from a kernel of three primitives — *satta* as the omnipresent ground, *activity* carrying the inseparable triad of effort, motion, and result, and *saturation* as the single relation between them. From that kernel this paper constructs the full tier-neutral structure of the darshan: units as bounded activity, relationships as definite couplings, value as what a coupling realises, composition as coupling that closes a new boundary, and transmission as the composition method recurring across member turnover — stated as one formal template of twenty definitions, seven laws, and six propositions, each grounded in the primary texts. A **recovery audit** classifies every clause as *derived* from the kernel, *definitional*, or *substantive textual data*, and a categorical formalisation supplies precise notation for the parts of the structure mathematics can carry — functors, enrichment, colimits, Petri processes, and an operadic sketch — while naming exactly where the notation stops.
...
**A** (activity). The dynamical primitive, carrying its triad internally: every activity has **effort** (*shram*), **motion** (*gati*), and **result** (*parinam*) as aspects, and each aspect is the joint form of the other two — "The outcome of motion and result is effort. The outcome of effort and motion is the result. The outcome of result and effort is motion" (MVD p. 105). The combined activity is *mool cheshta*, basic impulsion (MVD p. 40). Nothing about units, boundaries, or relationships is presupposed here: A is activity as such.

**Saturation.** The single primitive relation: every activity is in O — soaked, surrounded, submerged — and O is not an activity. "There is no provision in existence to separate units of nature from Omnipresence" (JV p. 18). Saturation is constitutive co-location, not containment of one thing inside another thing of the same kind. It names the first ontological relation — between the medium and every unit — distinct from unit-to-unit relationships, and it is not physical extraction or depletion of O: inherent energy and regulation belong to the saturated unit through the relation itself (SB pp. 49, 62).

### 2.2 Kernel axioms

**K1 (Energisation).** Every activity is energy-replete through saturation, and activity is neither created nor destroyed. The texts state the equation directly:
...
"Unit + Energy fullness = Activeness" (SB p. 69): one uniform energy manifests as the triad.

**K2 (Boundedness).** Activity clusters into bounded loci, and the bound is not imposed from outside — it is the closure of the activity's own motion-path:
...
The same closure that bounds also regulates: boundary and discipline are one structure, which is why the regulation ladder (D12) can begin inside the unit rather than descending on it.

**K3 (Coupling).** Activities couple: one activity's results enter another's as input, and no activity is uncoupled — "Nothing is isolated – that is the principle" (JV p. 43). The texts name the coupling's contribution explicitly:
...
Three assertions the texts state independently follow from the kernel and are recorded here so the audit can classify them. **Conservation:** existence is beginless and stable — "Existence neither increases nor decreases" (JV p. 127); results transform activity, they never annihilate it — a corollary of K1. **No isolation:** every unit participates in at least one relationship — the totality clause of K3, asserted directly at JV p. 43. **Inherent orderliness:** every unit is orderliness in itself and a participant in the overall orderliness (MVD p. 11; SB p. 13) — K2's closure is the unit's own orderliness, and participation follows from K3. There is no "loose matter" outside the structure.
...
| **Monoidal / Petri process** | Resource-sensitive transitions that consume inputs and produce outputs | The activity triad's bookkeeping; development progression (§7.10) |
...
The kernel supplies the primitives: **O**, the omnipresent medium; **A**, activity with its internal triad; and **saturation** between them. The derived sorts the clauses quantify over are **U**, the set of units (countable — "Nature is in the form of entities that can be counted", JV p. 45); **R ⊆ U × U**, relationships (*sambandh*, MVD p. 61); **V**, values (*mulya*) — the content exchanged in relationships (SB p. 50); **κ**, the composition operator (JV p. 67); and **τ**, the transmission operator (JV pp. 48, 51). O is not a member of U.
...
### 5.1 Units from bounded activity

Define a **unit** as a maximal bounded cluster of activity whose motion-path closes (K2). Everything D2 asserts follows in form: a unit is a bounded cluster of activity; insentient units are active within their bounds, sentient units beyond them (MVD p. 33) — in kernel terms, the closure can bound the activity's *extent* without bounding its *reach*. The signature sig(u) = ⟨roop, gun, svabhav, dharma⟩ splits by status: form (*roop*) is the geometry of the closed motion-path — derived; properties, essential nature, and *dharma* are what the texts assert about each order's activity — data organised by the construction, not produced by it. This is the kernel's strongest result: countability, boundedness, and the unit/medium distinction (O has no motion-path, hence is no unit) all fall out of K2 and saturation.
...
| Clause | Status | Notes |
|---|---|---|
| Saturation | kernel primitive | The saturation relation itself |
| K1–K5 | kernel axioms | Energisation, boundedness, coupling, recognition-as-law, completeness orientation |
| Conservation (§2.3) | derived | Corollary of K1: results transform, never annihilate |
...
| Inherent orderliness (§2.3) | derived | K2's closure is the unit's own orderliness; participation from K3 |
| D1 (activity triad) | kernel primitive | The structure of A |
| D2 (unit) | derived | C1 (§5.1); *roop* from boundary geometry |
...
| D14 (jeevan) | data | Constitutional completeness as the K5 result-goal is derived in form; the faculty inventory, orbital structure, and body–*jeevan* dyad are data; the reflexivity reading (§5.6) organises but does not derive them |
...
Independence of the kernel axioms is witnessed by countermodels in the intended informal semantics: without K2 there is undifferentiated activity and no unit — nothing for the rest of the template to be about; without K3 units are windowless and value has no slot; without K4 coupling is arbitrary and the definite orders lose their lawfulness; without K5 development has no direction and the natural/excited distinction collapses; without K1 activity floats free of the medium and the first ontological layer (saturation before *sambandh*) is lost. Formalising these countermodels — and mechanising the whole audit — is the continuation this paper's discipline makes possible but does not yet execute.
...
### 7.10 Development as a monoidal/Petri process

Development progression is resource-sensitive, best modelled by a **symmetric monoidal category** (equivalently the free such category on a Petri net), where the tensor is co-presence of resources and transitions consume inputs to produce outputs. This layer is the categorical home of the activity triad (D1): a firing carries effort, motion, and result *jointly* — effort consumed as input, result produced as output, the firing itself the motion — and φ reads value off the produced token. The Petri semantics is native to the triad's mutuality coupling: a transition's inputs legitimately include *other* units' outputs — one atom's expelled result enabling another's bonding (MVD p. 114). What the model does not carry is the triad's joint-form constitution — each moment the outcome of the other two (MVD p. 105) — which is a symmetric relation among aspects of one activity, not a sequence; it enters as transition *labelling*, not as composable arrows. Frustration (*kshobh*, D7) is an enabled transition that fails to fire: inputs present, output never produced — the triad not closing toward restfulness.
...
**The irreducible remainder.** O and saturation are primitives: the kernel begins from them and cannot ground them. The triad structure of A is taken from the texts whole — the kernel does not explain why activity has exactly these three aspects. The content of admissibility (which complementarities bond), the transmission carriers, the six kinds of value, the four orders' signatures, the *jeevan* faculty inventory, and the four human goals are taught content. Evaluation's restriction to *jeevan* is asserted, and the reflexivity reading organises it without deriving it.

**Jeevan's substantiality.** Category theory is structuralist: by Yoneda, an object just *is* its web of morphisms, so a faithful categorical reading inevitably re-describes *jeevan* as a functional role — precisely the position the darshan rejects. A kernel of activities re-describes *jeevan* functionally exactly as arrows and predicates did. This tension is a boundary of the tools, not a bug, and possibly unresolvable within them.
