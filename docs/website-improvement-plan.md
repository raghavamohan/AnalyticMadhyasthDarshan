...
- Author HTML passes a pinned HTML5 sanitizer before trusted reader chrome and KaTeX rendering. Static SVG validation rejects active content and external resources; metadata cannot close its JSON-LD script. Mermaid is pinned consistently to 11.15.0 in browser and PDF and uses strict security mode.
...
- Catalog dates use an explicit IST parser for ordering and document version URLs. The 45 existing study/companion readers were rebuilt; no canonical study Markdown, status or Edited-on timestamp was changed.
